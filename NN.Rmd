...
```{r nn2, echo=T, cache=T, dependson= c("nn1"), warning=FALSE, collapse=TRUE}
...
```{r nn4, echo=T, cache=T, dependson= c("nn3"), warning=FALSE}
...
```{r nn5, echo=T, cache=T,dependson= c("nn4"),warning=FALSE,collapse=TRUE}
...
From now we implement with sub-set with only sell==1, Implement NN for prediction on bid
...
```{r nn7, echo=T, cache=T,dependson= c("nn6"), warning=FALSE, collapse=TRUE}
classifier = h2o.deeplearning(y = 'biddy1',
                              training_frame = train,
                              validation_frame = valid,
                              activation = 'Rectifier',
